  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (res?.error) {
        ...
        setNotification(`Gagal menghubungkan ke Google: ${res.error}`);
      }
      // Browser akan langsung dialihkan ke accounts.google.com untuk memilih akun.
      // Sesi dashboard guru akan dimuat setelah proses otentikasi Google selesai.
    } catch (err: any) {
      setIsGoogleLoading(false);
      setNotification(`Gagal memulai login Google: ${err?.message || err}`);
    }
  };
            ...
            {(mode === 'login' || mode === 'signup') && (
              <div className="mt-5 space-y-4">
                <button
                  ...
                  className="w-full h-11 px-4 bg-white hover:bg-slate-50 border border-slate-200 hover:border-slate-300 rounded-xl flex items-center justify-center gap-2.5 text-slate-800 font-display font-bold text-xs transition-all shadow-xs cursor-pointer group disabled:opacity-75 disabled:cursor-not-allowed"
                >
                  {isGoogleLoading ? (
