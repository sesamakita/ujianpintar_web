...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      {/* 0. Mobile Backdrop Overlay (< lg only) */}
      {isMobileOpen && (
        <div
          className="fixed inset-0 bg-slate-950/50 backdrop-blur-xs z-40 lg:hidden transition-opacity duration-300 animate-in fade-in"
      ...
      <aside
        className={`
          w-72 bg-white border-r border-slate-200 flex flex-col h-screen select-none shadow-xs
          fixed lg:sticky top-0 z-50 lg:z-30 transition-transform duration-300 ease-in-out
      ...
      >
        ...
        <div className="p-4 border-b border-slate-100 flex items-center justify-between flex-shrink-0 bg-white">
          ...
          <div className="flex items-center gap-1">
            ...
            {/* Mobile Close Button (Hidden on Desktop: lg:hidden) */}
            {onCloseMobile && (
              <button
                type="button"
                onClick={onCloseMobile}
                title="Tutup Menu"
                className="lg:hidden p-2 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            )}
